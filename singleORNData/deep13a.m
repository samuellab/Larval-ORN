...
for i = 1:trialNum
    %fit
    [fitresult1, ~] = fit(log10(input.concList{1,1}), dataOdor1(i, :)', ft, opts);
    [fitresult2, ~] = fit(log10(input.concList{1,2}), dataOdor2(i, :)', ft, opts);
...
[slop, ampVec, kdVec] = EnsembleFit(dataX, dataY);
disp([num2str(mean(kdVec(1:12))), ' +- ',  num2str(std(kdVec(1:12))/sqrt(12))]);
disp([num2str(mean(kdVec(13:24))), ' +- ',  num2str(std(kdVec(13:24))/sqrt(12))]);
