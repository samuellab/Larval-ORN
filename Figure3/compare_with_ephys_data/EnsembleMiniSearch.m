...
options = optimset('TolFun',1e-4,'Display','notify','MaxIter',1000000,'MaxFunEvals',1000000 );% ,'PlotFcns',@optimplotfval);
[param, ssr]= fminsearch(eSSR, p0, options);

slop= param(1);
% ampVec = param(2:trialNum+1);
ampVec = param(2);
kdVec  = param(3 : trialNum+2);
...
slop= param(1);
amp = param(2);
kd  = param(3: trialNum+2);

yHat = zeros(trialNum, samples);

for i = 1:trialNum
    yHat(i, :) = hillEq(amp, slop, kd(i), x(i, :));
end
